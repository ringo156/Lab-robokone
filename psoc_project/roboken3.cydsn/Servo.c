...
void Servo_Dataset(Servo_Data* servo, uint8 id, uint8 speed, uint8 stretch, int angle, int16 max, int16 min){
    servo->id = id;
    servo->speed = speed;
...
void angle_set(Servo_Data* servo, int16 angle){
    ...
    unsigned char tx[3];
    char buf[32];
    if(servo->angle > servo->angle_max)
	{
		return;
	}
	else if(servo->angle < servo->angle_min)
	{
		return;
	}
